...
function SkibidicordPopout(onClose: () => void) {
    const { useQuickCss } = useSettings(["useQuickCss"]);

    const pluginEntries = [] as ReactNode[];

    for (const plugin of Object.values(Skibidicord.Plugins.plugins)) {
        if (plugin.toolboxActions && Skibidicord.Plugins.isPluginEnabled(plugin.name)) {
            pluginEntries.push(
                <Menu.MenuGroup
                    label={plugin.name}
...
function SkibidicordPopoutIcon(isShown: boolean) {
    return (
...
function SkibidicordPopoutButton() {
    ...
    return (
        <Popout
            ...
            renderPopout={() => SkibidicordPopout(() => setShow(false))}
        >
            {(_, { isShown }) => (
                <HeaderBarIcon
                    className="vc-toolbox-btn"
                    onClick={() => setShow(v => !v)}
                    tooltip={isShown ? null : "Skibidicord Toolbox"}
                    icon={() => SkibidicordPopoutIcon(isShown)}
                    selected={isShown}
                />
            )}
        </Popout>
...
export default definePlugin({
    name: "SkibidicordToolbox",
    description: "Adds a button next to the inbox button in the channel header that houses Skibidicord quick actions",
