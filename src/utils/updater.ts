...
export async function checkForUpdates() {
    changes = await Unwrap(SkibidicordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(SkibidicordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(SkibidicordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(SkibidicordNative.updater.getRepo());
