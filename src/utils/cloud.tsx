...
export async function getAuthorization() {
    const secrets = await DataStore.get<Record<string, string>>("Skibidicord_cloudSecret") ?? {};

    const origin = cloudUrlOrigin();

    // we need to migrate from the old format here
    if (secrets[origin]) {
        await DataStore.update<Record<string, string>>("Skibidicord_cloudSecret", secrets => {
            secrets ??= {};
