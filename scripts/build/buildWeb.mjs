...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/Skibidicord.ts"],
...
await Promise.all(
    [
        ...
        esbuild.build({
            ...
            footer: { js: "//# sourceURL=SkibidicordWeb" }
        }),
        esbuild.build({
            ...commonOptions,
        ...
        esbuild.build({
            ...
            outfile: "dist/Skibidicord.user.js",
            banner: {
                js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
            },
            footer: {
                // UserScripts get wrapped in an iife, so define Skibidicord prop on window that returns our local
                js: "Object.defineProperty(unsafeWindow,'Skibidicord',{get:()=>Skibidicord});"
            }
        })
    ]
).catch(err => {
...
async function buildExtension(target, files) {
    const entries = {
        "dist/Skibidicord.js": await readFile("dist/extension.js"),
        "dist/Skibidicord.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/Skibidicord.user.css", "utf-8").then(content => {
    const cssRuntime = `
;document.addEventListener("DOMContentLoaded", () => document.documentElement.appendChild(
    Object.assign(document.createElement("style"), {
        textContent: \`${content.replaceAll("`", "\\`")}\`,
        id: "skibidicord-css-core"
    })
), { once: true });
`;

    return appendFile("dist/Skibidicord.user.js", cssRuntime);
});
