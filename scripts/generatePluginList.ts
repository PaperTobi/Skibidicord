...
interface PluginData {
    ...
    target: "discordDesktop" | "skibidicordDesktop" | "desktop" | "web" | "dev";
    filePath: string;
}
...
async function parseFile(fileName: string) {
    ...
    for (const node of file.getChildAt(0).getChildren()) {
        ...
        if (target) {
            if (!["web", "discordDesktop", "skibidicordDesktop", "desktop", "dev"].includes(target)) throw fail(`invalid target ${target}`);
